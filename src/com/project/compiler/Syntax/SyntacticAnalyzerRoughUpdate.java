 ...
 *
 */
public class SyntacticAnalyzerRoughUpdate {
	 ...
	 * @param lexer
	 */
	public SyntacticAnalyzerRoughUpdate(LexicalAnalyzer lexer) {
		this.lexer = lexer;
...
//				}
	 ...
	 * @return
	 */
	private boolean expr(AstNode exprNode) {
		AstNode arithExprNode = new ArithExprNode();
		AstNode relExprNode = new RelExprNode();
	 ...
	 * @return
	 */
	private boolean exp_Tail(AstNode arithExprNode, AstNode relExprNode) {
	 ...
	 * @return
	 */
	private boolean relExpr(AstNode relExprNode)
	{
		AstNode relop = new IdNode();
		AstNode arithExprNode = new ArithExprNode();
		AstNode _arithExprNode = new ArithExprNode();
	 ...
	 * @return
	 */
	private boolean arithExpr(AstNode arithExprNode)
	{
		...
		if (checkFirstSet("term")) {
			...
			if (
					term(termNode)
					&& arithExpr_Tail(termNode, arithExprNode, false)) {
	 ...
	 * @return
	 */
	private boolean  arithExpr_Tail(AstNode termNode, AstNode arithExprNode, boolean childAddedInParent)
	{
		...
		if (checkFirstSet("addOp")) {
			...
			if (matchTokenType("addOp", addOp) 
					&& term(_termNode)
					&& arithExpr_Tail(_termNode, arithExprNode, true)) {
...
//		} 
	 ...
	 * @return
	 */
	private boolean term(AstNode termNode)
	{
		...
		if (checkFirstSet("factor")) {
			...
			if (factor(factorNode) && 
					term_Tail(factorNode, termNode, false)) {
	 ...
	 * @return
	 */
	private boolean term_Tail(AstNode factorNode, AstNode termNode, boolean childAddedInParent)
	{
		...
		if (checkFirstSet("multOp")) {
			...
			if (matchTokenType("multOp", multOp)
					&& factor(_factorNode)
					&& term_Tail(_factorNode, termNode, true)) {
...
//		}
	 ...
	 * @return
	 */
	private boolean factor(AstNode factorNode)
	{
...
//				}				
	 ...
	 * @return
	 */
	private boolean functionCall(AstNode var)
	{
	 ...
	 * @return
	 */
	private boolean variable(AstNode var)
	{
	 ...
	 * @return
	 */
	private boolean variable_Tail(AstNode id, AstNode varElement)
	{
	 ...
	 * @return
	 */
	private boolean idnest_List_Tail(AstNode varElement)
	{
	 ...
	 * @return
	 */
	private boolean  idnest_(AstNode varElementNode)
	{
		AstNode id = new IdNode();
	 ...
	 * @return
	 */
	private boolean  idnest_Tail(AstNode id, AstNode varElementNode)
	{
		AstNode rlp = new IdNode(), rrp = new IdNode();
		AstNode fCallNode =  new  FCallNode(); 
		AstNode dataMemberNode = new DataMemberNode();
