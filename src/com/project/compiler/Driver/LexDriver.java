...
public class LexDriver extends baseDriver {
	...
	public void execute(String path, int index) {
		try {
			...
			// Save Tokens
			String tokenOutputPath = String.format(genericPath, index,
					Enums.ModuleType.LEXICAL.toString() + "_Output.txt");
			...
			String errorOutputPath = String.format(genericPath, index,
					Enums.ModuleType.LEXICAL.toString() + "_ErrorOutput.txt");
			String errorsResultText = printTokens("Errors", lexicalAnalyzer.getErrorTokenList(),
					TokenPrintFormat.REQUIRED);
			...
			// Save AtoCC
			String atoCCPath = String.format(genericPath, index,
					Enums.ModuleType.LEXICAL.toString() + "_AtoCCOutput.txt");
